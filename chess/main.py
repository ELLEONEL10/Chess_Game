...
def pawnPromotionPopup(screen, gs):
    font_path = os.path.join('chess', 'font.otf')
    font = p.font.Font(font_path, 22)  # Use your specified font for general text
    moveLogFont = font  # Use the same font for move logs
    big_font = p.font.Font(font_path, 45)
...
def main():
    ...
    screen = p.display.set_mode(
        (BOARD_WIDTH + MOVE_LOG_PANEL_WIDTH, BOARD_HEIGHT + 70))  # Added 100 pixels for the bottom bar
    clock = p.time.Clock()
    screen.fill(p.Color(LIGHT_SQUARE_COLOR))
    font_path = os.path.join('chess', 'font.otf')
    font = p.font.Font(font_path, 22)  # Use your specified font for general text
    ...
    bottom_bar = p.Rect(0, BOARD_HEIGHT, BOARD_WIDTH, bottom_bar_height)

    # Button definitions within the bottom bar
    forfeit_button = p.Rect(50, BOARD_HEIGHT + 25, 120, 40)
    main_menu_button = p.Rect(200, BOARD_HEIGHT + 25, 120, 40)
    buttons = {
    ...
    while running:
        ...
        if moveMade:
            ...
            animate = False
            moveUndone = False
        drawGameState(screen, gs, validMoves, squareSelected, moveLogFont, buttons, font, bottom_bar)
        if COUNT_DRAW == 1:
            gameOver = True
            text = 'Draw due to repetition'
            drawEndGameText(screen, text, font_path, big_font)
        if gs.stalemate:
            gameOver = True
            text = 'Stalemate'
            drawEndGameText(screen, text, font_path, big_font)
...
def drawGameState(screen, gs, validMoves, squareSelected, moveLogFont, buttons, font, bottom_bar):
    drawSquare(screen)
    ...
    # Draw the bottom bar
    p.draw.rect(screen, p.Color('gray'), bottom_bar)  # Draw a gray bar at the bottom

    # Render Buttons on the bottom bar
    forfeit_text = font.render('FORFEIT', True, p.Color('black'))
    main_menu_text = font.render('Main Menu', True, p.Color('black'))
    p.draw.rect(screen, p.Color('white'), buttons["FORFEIT"])
    p.draw.rect(screen, p.Color('white'), buttons["Main Menu"])
    screen.blit(forfeit_text, (buttons["FORFEIT"].x + 10, buttons["FORFEIT"].y + 7.5))
    screen.blit(main_menu_text, (buttons["Main Menu"].x + 7.5, buttons["Main Menu"].y + 7.5))

def drawEndGameText(screen, text, font_path, big_font):
    font = p.font.Font(font_path, 30)  # Or use 'big_font' if you want to utilize a specific style
    text_surface = big_font.render(text, True, p.Color("white"))
    text_width, text_height = text_surface.get_size()

    background_rect = p.Rect((BOARD_WIDTH - text_width) // 2 - 10,
                             (BOARD_HEIGHT - text_height) // 2 - 10,
                             text_width + 20, text_height + 20)
    p.draw.rect(screen, p.Color("black"), background_rect)
    screen.blit(text_surface, ((BOARD_WIDTH - text_width) // 2, (BOARD_HEIGHT - text_height) // 2))
